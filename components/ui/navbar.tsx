...
import { 
  RiCloseFill, RiMenu3Fill, RiSearchLine, 
  RiBookmark3Fill, RiNotification3Line, RiUser3Line 
} from "react-icons/ri";
import Link from "next/link";

// Added specific utility links to make it 'Essential'
const LINKS = [
  { name: 'Originals', link: '/originals' },
  { name: 'Library', link: '/library', icon: <RiBookmark3Fill className="inline mr-1" /> },
    ...
    <nav className={`fixed w-full top-0 z-[100] transition-all duration-500 ${
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between relative">
        ...
        <div className={`flex items-center justify-end transition-all duration-300 ${searchActive ? 'flex-1' : 'gap-2 md:gap-4'}`}>
            ...
            searchActive ? 'w-full md:w-96 px-4 md:px-0' : 'w-10'
             ...
             <input 
                ...
                className={`h-11 rounded-2xl bg-white border border-stone-200 text-xs font-bold text-stone-900 focus:outline-none focus:ring-4 focus:ring-[#F56476]/10 focus:border-[#F56476] transition-all duration-500 ${
                  searchActive ? 'w-full pl-5 pr-12 opacity-100 scale-100 shadow-xl' : 'w-0 opacity-0 scale-95 pointer-events-none'
                }`}
             ...
             <button 
                ...
                className={`z-20 p-2.5 transition-all duration-500 rounded-xl ${
                    searchActive ? 'absolute right-6 md:right-1 bg-[#102321] text-white shadow-lg rotate-90' : 'text-stone-500 hover:bg-white hover:text-[#F56476] hover:shadow-md'
                }`}
             >
          ...
          {/* Identity & Publish - The Essential Utility */}
          <div className={`hidden md:flex items-center gap-5 text-[11px] font-black text-stone-600 ml-4 ${searchActive ? 'hidden xl:flex' : ''}`}>
            <Link href="/profile" className="hover:text-[#F56476] transition-all hover:-translate-y-0.5 tracking-widest flex items-center gap-1">
              <RiUser3Line size={16} /> MY ODYSSEY
            </Link>
            <Link 
              href="/publish" 
              className="bg-[#102321] text-white px-7 py-3 rounded-xl hover:bg-[#F56476] hover:shadow-[0_10px_20px_rgba(245,100,118,0.3)] transition-all shadow-md active:scale-95 tracking-widest italic"
            >
              PUBLISH
            </Link>
          </div>
          ...
          <div className={`lg:hidden relative z-[110] ${searchActive ? 'hidden' : 'block'}`}>
            <button onClick={toggleMenu} className={`text-stone-900 p-2 rounded-xl transition-all duration-300 ${isOpen ? 'bg-[#F56476] text-white rotate-180' : 'bg-transparent'}`}>
      ...
      {/* --- Mobile Overlay --- */}
      <div className={`lg:hidden fixed inset-0 bg-[#edf7f6] z-[100] transition-all duration-500 ease-in-out ${isOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
        <div className="px-8 pt-32 pb-10 flex flex-col h-full justify-between">
          <div className="flex flex-col gap-8">
            {LINKS.map((link, index) => (
              <Link key={index} href={link.link} onClick={() => setIsOpen(false)} className="text-5xl font-black uppercase italic text-stone-900 hover:text-[#F56476] tracking-tighter transition-all hover:translate-x-4">
                {link.name}
              </Link>
            ))}
          </div>
          <div className="space-y-6 pb-12">
            ...
            <Link href="/publish" className="block text-2xl font-black text-[#F56476] uppercase italic tracking-tighter">START CREATING</Link>
            <Link href="/profile" className="block text-xl font-black text-stone-400 uppercase italic tracking-tighter">Account Settings</Link>
          </div>
        </div>
      </div>
    </nav>
